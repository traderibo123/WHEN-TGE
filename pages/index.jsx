...
export default function Home() {
  ...
  const loadImage = useCallback((url) => new Promise((res, rej) => {
    const img = new Image();
  ...
  const drawTextWhenTGE = useCallback((ctx) => {
  ...
  const circleImage = useCallback((ctx, img, cx, cy, r) => {
  ...
  const loadAvatarByHandle = useCallback(async (raw) => {
  ...
  const loadProjectLogo = useCallback(async (s, nameFallback = "P") => {
    ...
    const letter = (nameFallback && nameFallback[0]) || "P"; return placeholderCircle(letter, "#7c5cff", 200);
  ...
  const renderCanvas = useCallback(async () => {
    ...
    const logoImg = await loadProjectLogo(slug, proj && proj.name);
    circleImage(ctx, logoImg, LOGO_POS.x, LOGO_POS.y, LOGO_R);
  }, [AVATAR_POS.x, AVATAR_POS.y, LOGO_POS.x, LOGO_POS.y, userHandle, slug, drawTextWhenTGE, loadAvatarByHandle, placeholderCircle, circleImage, loadProjectLogo]);
      ...
      <div style={{ display:"flex", gap:10, flexWrap:"wrap", marginTop:12 }}>
        ...
        <label style={{ position:"relative", overflow:"hidden", borderRadius:10, padding:"11px 14px", border:"1px solid #222634", background:"#131826", color:"#fff", fontWeight:600, cursor:"pointer" }}>
          ...
          <input type="file" accept=".png,.svg,.jpg,.jpeg,.webp"
                 onChange={(e)=>{ const f = e.target.files?.[0]; if(!f) { setCustomLogo(null); return; } const r = new FileReader(); r.onload = ev => setCustomLogo(String(ev.target.result)); r.readAsDataURL(f); }}
